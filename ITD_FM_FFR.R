...
dat$FMave <- (dat$FMleftMean + dat$FMrightMean)/2.0
m_itd_ffr_lat = lm(ITDmean ~ mistakeITD + FFR1kmagdB + lat180dB, data = dat, na.action = na.exclude)
m_itd_lat = lm(ITDmean ~ mistakeITD + lat180dB, data = dat, na.action = na.exclude)
m_itd_audiogram_lat = lm(ITDmean ~ mistakeITD + lat180dB + HL500 + HFA, data=dat, na.action = na.exclude)
# m_itd_audiogram_nomistake_lat = lm(ITDmean ~ lat180dB + HL500 + HFA, data=dat, na.action = na.exclude)
